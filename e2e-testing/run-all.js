import { spawn } from 'child_process';
import http from 'http';
import path from 'path';
import { fileURLToPath } from 'url';
import { runSeleniumTests } from './selenium-web/selenium-runner.js';
import { runAppiumTests } from './appium-mobile/appium-runner.js';
import { ExcelReporter } from './utils/excel-reporter.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Helper to check if server is reachable
function checkServer(url) {
  return new Promise((resolve) => {
    http.get(url, (res) => {
      resolve(res.statusCode === 200 || res.statusCode === 302);
    }).on('error', () => {
      resolve(false);
    });
  });
}

// Helper to wait for the Vite dev server to boot
async function waitForServer(url, timeoutMs = 15000) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
    const isUp = await checkServer(url);
    if (isUp) return true;
    await new Promise((r) => setTimeout(r, 1000));
  }
  return false;
}

async function main() {
  console.log('=====================================================');
  console.log('       GramVoice End-to-End Test Suite Orchestrator   ');
  console.log('=====================================================\n');

  const rootDir = path.resolve(__dirname, '..');
  const baseUrl = 'http://localhost:5173';
  let devServerProcess = null;

  try {
    // 1. Check if server is already running, if not start it
    const alreadyRunning = await checkServer(baseUrl);
    
    if (alreadyRunning) {
      console.log(`Vite server already detected running at ${baseUrl}. Re-using session.`);
    } else {
      console.log('Starting local Vite development server...');
      
      // Spawn Vite command from root workspace
      devServerProcess = spawn('npx', ['vite'], {
        cwd: rootDir,
        shell: true,
        stdio: 'pipe'
      });

      // Log errors or outputs if needed for debugging
      devServerProcess.stderr.on('data', (data) => {
        console.error(`[Vite Server Error]: ${data}`);
      });

      // Wait for server to boot
      console.log('Waiting for Vite server port to become active...');
      const serverStarted = await waitForServer(baseUrl, 15000);
      if (!serverStarted) {
        throw new Error('Vite dev server failed to start or respond within 15 seconds.');
      }
      console.log(`Vite server is running and ready at ${baseUrl}.\n`);
    }

    // 2. Execute Web (Selenium) test suite
    const webResults = await runSeleniumTests(baseUrl);

    // 3. Execute Mobile (Appium) test suite
    const mobileResults = await runAppiumTests(baseUrl);

    // 4. Combine results and generate Excel report
    const allResults = [...webResults, ...mobileResults];
    console.log(`Consolidating ${allResults.length} total test results...`);

    const reporter = new ExcelReporter(path.join(__dirname, 'reports'));
    const reportPath = await reporter.generateReport(allResults);

    console.log('\n=====================================================');
    console.log('               Execution Summary                      ');
    console.log('=====================================================');
    console.log(`Total Cases Run: ${allResults.length}`);
    console.log(`Web (Selenium):  ${webResults.length}`);
    console.log(`Mobile (Appium): ${mobileResults.length}`);
    console.log(`Passed:          ${allResults.filter(r => r.status === 'PASS').length}`);
    console.log(`Failed:          ${allResults.filter(r => r.status === 'FAIL').length}`);
    console.log(`Skipped:         ${allResults.filter(r => r.status === 'SKIPPED').length}`);
    console.log(`Excel Report:    ${reportPath}`);
    console.log('=====================================================\n');

  } catch (error) {
    console.error('CRITICAL ERROR in orchestrator run:', error);
  } finally {
    // 5. Clean up Vite server if we started it
    if (devServerProcess) {
      console.log('Stopping local Vite server process...');
      devServerProcess.kill();
      // On Windows, child process spawn shell requires killing process trees sometimes.
      // We can also run a taskkill or rely on node's signal sending.
      try {
        if (process.platform === 'win32') {
          spawn('taskkill', ['/pid', devServerProcess.pid, '/f', '/t'], { shell: true });
        }
      } catch (e) {
        // ignore taskkill errors
      }
      console.log('Vite server process terminated.');
    }
    
    process.exit(0);
  }
}

main();
